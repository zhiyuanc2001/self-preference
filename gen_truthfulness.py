...
from prompts.truthfulness import(
    TRUTHFULNESS_USER_PROMPT,
    TRUTHFULNESS_PROMPT_TWO_SHOT
)
...
def format_dataset_instruct_model(data, meta_user_prompt: str):
    ...
    for item in data:
        user_prompt = meta_user_prompt.format(query=item["question"])
        msg = [
            {"role": "user", "content": user_prompt}
        ]
        messages.append(msg)
        ids.append(item["id"])
        golden_responses.append(item["best_answer"])
...
def format_dataset_base_model(data, prompt: str):
    messages, ids, golden_responses, queries = [], [], [], []
    for item in data:
        msg = prompt.format(test_query=item["question"])
        messages.append(msg)
        ids.append(item["id"])
        golden_responses.append(item["best_answer"])
        queries.append(item["question"])
...
def generate_truthfulness(data_dir, model_type, is_instruct, batch_size):
    """
        generate truthfulness answer for truthful_qa task.
    ...
    save_path = f"model_responses_fullset/truthfulness/{data_type}/{model_name}.jsonl"
    # load data
    test_data = load_from_disk(data_dir)
    
    test_data = test_data.select(range(530))  # select 500 in the post-process stage
    print(f'Test data numbers: {len(test_data)}')

    if data_type == "truthful_qa":
        if is_instruct == True:
            print("Instruct Model")
            meta_user_prompt = TRUTHFULNESS_USER_PROMPT
            messages, ids, golden_responses, queries = format_dataset_instruct_model(test_data, meta_user_prompt)
            stop_words = None
        else:
            print("Base Model")
            prompt = TRUTHFULNESS_PROMPT_TWO_SHOT
